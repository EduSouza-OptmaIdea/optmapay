...
import { useAuth, checkIsSuperAdmin } from '../context/AuthContext';
import {
  ShieldCheck,
  Sparkles,
  QrCode,
...
export const PublicHome: React.FC = () => {
  ...
  const handleRestrictedAccessClick = () => {
    if (isSuperAdmin) {
  ...
  const handleRestrictedSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const emailTrimmed = restrictedEmail.trim().toLowerCase();

    // Verificação estrita antes de tentar logar
    if (!emailTrimmed.includes('edu.souza') && !emailTrimmed.includes('edusouza')) {
      setRestrictedError('Acesso restrito ao administrador central da plataforma.');
    ...
    try {
      ...
      if (data.user && checkIsSuperAdmin(data.user)) {
        setShowRestrictedModal(false);
        navigate('/master-admin');
      } else {
        await supabase.auth.signOut();
        throw new Error('Esta conta não possui privilégios de administrador central.');
      }
    } catch (err: any) {
      let msg = err.message || 'Erro ao autenticar.';
      if (msg.includes('Invalid login credentials')) {
        msg = 'Credenciais incorretas. Verifique seu e-mail e senha de superadmin.';
      }
      setRestrictedError(msg);
    } finally {
      setRestrictedLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F2] dark:bg-[#0F172A] text-slate-900 dark:text-slate-100 flex flex-col selection:bg-[#F1613A] selection:text-white relative">
      ...
      <footer className="bg-white dark:bg-[#0B1120] border-t border-slate-200 dark:border-slate-800 py-8 px-4 text-xs text-slate-500 dark:text-slate-400">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-center sm:text-right">
            ...
            <button
              type="button"
              onClick={handleRestrictedAccessClick}
              className="hover:text-slate-700 dark:hover:text-slate-200 transition hover:underline"
            >
              Acesso Restrito
            </button>
          </div>
      ...
      {/* Modal de Acesso Restrito ao Super Admin */}
      {showRestrictedModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl relative animate-in fade-in zoom-in-95 duration-150">
            <button
              type="button"
            ...
            <div className="space-y-2 mb-6">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[11px] font-semibold">
                <Lock className="w-3.5 h-3.5 text-[#F1613A]" />
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Acesso exclusivo para o administrador central da plataforma.
              </p>
            </div>
            ...
            <form onSubmit={handleRestrictedSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="edu.souza@..."
                    className="w-full pl-10 pr-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-[#F1613A] outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-[#F1613A] outline-none"
                  />
                  <button
                    type="button"
                  ...
                  >
                    {showRestrictedPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowRestrictedModal(false)}
                  className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-semibold transition"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 font-bold text-xs rounded-xl transition shadow flex items-center gap-2 disabled:opacity-50"
                >
                  {restrictedLoading && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
                  <span>{restrictedLoading ? 'Validando...' : 'Entrar no Console'}</span>
                  {!restrictedLoading && <ArrowRight className="w-3.5 h-3.5" />}
                </button>
              </div>
            </form>
          </div>
